"""
tools/proposals.py

Gestion des propositions commerciales Dolibarr
"""

from datetime import datetime

from dolibarr_client import client
from database import Database



def list_proposals():
    """
    Liste toutes les propositions commerciales.
    """

    return client.get(
        "api/index.php/proposals"
    )



def create_proposal(
    client_id,
    note_public="Créé via Dolibarr MCP"
):
    """
    Crée une proposition commerciale
    pour un client existant.
    """

    data = {
        "socid": int(client_id),
        "date": int(datetime.now().timestamp()),
        "note_public": note_public,
        "remise_percent": 0
    }


    response = client.post(
        "api/index.php/proposals",
        data
    )


    # Dolibarr retourne généralement l'id créé
    if isinstance(response, dict):

        if "id" in response:
            return int(response["id"])


    return response



def add_proposal_line(
    proposal_id,
    product_id,
    quantity,
    price,
    description=""
):
    """
    Ajoute une ligne produit à une proposition commerciale.
    
    NOTE: Il y a un bug dans l'API Dolibarr pour les lignes de devis.
    L'API crée bien la ligne mais n'enregistre pas les champs.
    Cette fonction applique un correctif automatique en mettant à jour
    les champs directement en base de données.
    """

    data = {
        "fk_product": int(product_id),
        "qty": float(quantity),
        "subprice": float(price),
        "tva_tx": 0.0,
        "remise_percent": 0,
        "desc": description,
        "price": float(price),
        "price_ttc": float(price)
    }

    response = client.post(
        f"api/index.php/proposals/{int(proposal_id)}/lines",
        data
    )
    
    # Workaround: L'API crée la ligne mais ne remplit pas les champs
    # On doit les mettre à jour directement en BD
    if isinstance(response, str) and response.strip():
        try:
            # Extraire le rowid de la réponse XML
            line_id = int(response.strip().split('\n')[-1])
            
            # Mettre à jour les champs en base de données
            db = Database()
            update_sql = f"""
            UPDATE llx_propaldet 
            SET fk_product = {int(product_id)}, 
                qty = {float(quantity)}, 
                subprice = {float(price)}, 
                tva_tx = 0, 
                remise_percent = 0
            WHERE rowid = {line_id}
            """
            db.execute(update_sql)
            
            return response
        except:
            # Si quelque chose échoue, retourner la réponse originale
            return response
    
    return response


def validate_proposal(
    proposal_id
):
    """
    Valide une proposition commerciale.
    """

    proposal_id = int(proposal_id)


    data = {
        "notrigger": 0
    }


    return client.post(
        f"api/index.php/proposals/{proposal_id}/validate",
        data
    )